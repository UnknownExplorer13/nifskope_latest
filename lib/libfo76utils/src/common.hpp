...
class NifSkopeError : public std::exception
{
 ...
 public:
  NifSkopeError() noexcept
  {
    s = defaultErrorMessage;
    buf = nullptr;
  }
  NifSkopeError(const NifSkopeError& r) noexcept
  {
    storeMessage(r.s, bool(r.buf));
  }
  NifSkopeError(int copyFlag, const char *msg) noexcept
  {
...
#endif
  NifSkopeError(const char *fmt, ...) noexcept;
  virtual ~NifSkopeError() noexcept;
  NifSkopeError& operator=(const NifSkopeError& r) noexcept
  {
    storeMessage(r.s, bool(r.buf));
    return (*this);
  }
